...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    public float spacingBetweenPages = 0f;
    public AudioSource swipeMusic;
    ...
    void Start()
    {
        ...
        {
            EditorDebugger.LogWarning("Confusing setting of horizontal/vertical direction. Default set to horizontal.");
            _horizontal = true;
        }
    ...
    private void SetPagePositions()
    {
        ...
        for (int i = 0; i < _pageCount; i++)
        {
            ...
            if (_horizontal)
            {
                childPosition = new Vector2((i * (width - spacingBetweenPages) - containerWidth / 2) + offsetX, 0f);
            }
            else
            {
                childPosition = new Vector2(0f, -(i * (height - spacingBetweenPages) - containerHeight / 2 + offsetY));
            }
    ...
    {
        if (this.swipeMusic != null)
            this.swipeMusic.Play();
    ...
    private void InitPageSelection()
    {
        ...
        if (_showPageSelection)
        {
            ...
            {
                EditorDebugger.LogWarning("Different count of pages and selection icons - will not show page selection");
            ...
            else
            {
                ...
                for (int i = 0; i < pageSelectionIcons.childCount; i++)
                {
                    ...
                    if (image == null)
                    {
                        EditorDebugger.LogWarning("Page selection icon at position " + i + " is missing Image component");
                    }
